...
class CommissionBuy(models.Model):
    ...
    flag = models.BooleanField(default=False,verbose_name=u'是否被处理',db_index=True)
    status = models.IntegerField(choices=COMMISSION_STATUS_CHOICES, db_index=True, verbose_name=u'状态')
    time_value = models.BigIntegerField(default=0, blank=True, null=True)
    created_datetime = models.DateTimeField(auto_now_add=True, verbose_name=u'创建时间')
    ...
    def _commission_no(self):
        return "B%08.f"%self.id
    commission_no = property(_commission_no)
...
class TradeComplete(models.Model):
    ...
    order = models.ForeignKey('ProductOrder', blank=True, null=True, related_name='trade_order', db_index=True,
                                verbose_name=u'订单')
...
#
#     def _trade_no(self):
#         return "T%08.f"%self.id
#     trade_no = property(_trade_no)
...
class StockTicker(models.Model):
    ...
    total = models.FloatField(default=0, blank=True, null=True, verbose_name=u'成交金额')
...
class UserMoneyChange(models.Model):
    ...
    order_no = models.CharField(blank=True, null=True, max_length=255, verbose_name=u'订单编号')
    ...
    market_trade_serial_no =  models.CharField(blank=True, null=True, max_length=100, verbose_name=u'充值提现市场流水号')  # 充值提现时，为向银行请求的市场流水号  add by lwj 20151215
    desc = models.CharField(default='', blank=True, null=True, max_length=1000, verbose_name=u'备注')
    created_date = models.DateField(auto_now_add=True, db_index=True, verbose_name=u'创建日期')
    created_time = models.TimeField(auto_now_add=True, verbose_name=u'创建时间')
    modified_date = models.DateField(auto_now=True, verbose_name=u'修改日期')
    ...
    def __unicode__(self):
            return self.user.username
    def custom_save(self, *args, **kwargs):
        is_insert = True if self.id in (None, '') else True
        super(UserMoneyChange, self).save(*args, **kwargs)
        freezing_money = float(self.price)
        today = datetime.datetime.today().date()
        #进货买货冻结
        if self.trade_type in [3,6]:
            # 余额变化
            user_balance = UserBalance.objects.get_or_create(user=self.user)[0]
            user_balance.balance -= freezing_money
            user_balance.locked += freezing_money
            user_balance.save()
            #资产日报
            ...
            if self.trade_type==6:
                ...
                user_product.quote_quantity -= float(self.commission_buy_quantity)
                user_product.save()
        #进货购买解冻
        ...
        #进货购买成交
            ...
            #资产日报
            daily_report = UserAssetDailyReport.objects.get_or_create(user=self.user,target_date=today)[0]
            daily_report.expenditure += freezing_money
            ...
            if self.trade_type==5:
                ...
                user_product = UserProduct.objects.get_or_create(user=self.user,product=self.product,trade_type=1)[0]
                user_product.quote_quantity += self.trade_quantity*product_config.quote
            elif self.trade_type==8:
                user_product = UserProduct.objects.get_or_create(user=self.user,product=self.product,trade_type=2)[0]
                user_product.total_buy_quantity += self.trade_quantity
            user_product.total += self.price
            user_product.quantity += self.trade_quantity
            user_product.can_pickup_quantity += self.trade_quantity
...
#             user_product.need_repayment_amount
        ...
        #出售成功
            ...
            #资产日报
            daily_report = UserAssetDailyReport.objects.get_or_create(user=self.user,target_date=today)[0]
            daily_report.income += freezing_money
            daily_report.end_balance += freezing_money
            daily_report.can_use_amount += freezing_money
            daily_report.can_out_amount += freezing_money
            daily_report.total += freezing_money
        ...
        #卖手续费
            ...
            # 余额变化
            user_balance = UserBalance.objects.get_or_create(user=self.user)[0]
            user_balance.balance -= self.price
            user_balance.save()
            #资产日报
            daily_report = UserAssetDailyReport.objects.get_or_create(user=self.user,target_date=today)[0]
            daily_report.can_use_amount -= self.price
            daily_report.end_balance -= self.price
            daily_report.can_out_amount -= self.price
            daily_report.save()
        #收取手续费
        elif self.trade_type == 21:
            # 余额变化
            user_balance = UserBalance.objects.get_or_create(user=self.user)[0]
            user_balance.balance += self.price
            user_balance.save()
        ...
        #撤单
            ...
            user_balance = UserBalance.objects.get_or_create(user=self.user)[0]
            user_balance.balance += self.price
            user_balance.locked -= self.price
            user_balance.save()
            #资产日报
            daily_report = UserAssetDailyReport.objects.get_or_create(user=self.user,target_date=today)[0]
            daily_report.locked -= self.price
            daily_report.can_use_amount += self.price
            daily_report.end_balance += self.price
            daily_report.can_out_amount += self.price
            daily_report.save()
        #订单快递费
        elif self.trade_type == 16:
            # 余额变化
            user_balance = UserBalance.objects.get_or_create(user=self.user)[0]
            user_balance.balance -= float(self.price)
            user_balance.save()
            #资产日报
            daily_report = UserAssetDailyReport.objects.get_or_create(user=self.user,target_date=today)[0]
            daily_report.can_use_amount -= float(self.price)
            daily_report.end_balance -= float(self.price)
            daily_report.can_out_amount -= float(self.price)
            daily_report.save()
        #订单杂费
        elif self.trade_type == 17:
            # 余额变化
            user_balance = UserBalance.objects.get_or_create(user=self.user)[0]
            user_balance.balance -= float(self.price)
            user_balance.save()
        ...
        # 提现 by lzy 20151104
        elif self.trade_type == 2 and self.status in [1,2,3]:
            num = self.price
            ...
            if is_insert:
                if self.status in [1, 2]:
                    ...
                    user_balance = UserBalance.objects.get(user=self.user)
                    user_balance.balance = F('balance') - num
                    if self.status == 1: #进行中
                        user_balance.locked = F('locked') + num
                    user_balance.save()
                    #资产日报
                    daily_report = UserAssetDailyReport.objects.get_or_create(user=self.user,target_date=today)[0]
                    ...
                    if self.status == 1:    #进行中
                        daily_report.locked = F('locked') + num
                    else:   # ①
                        daily_report.expenditure = F('expenditure') + num
                        daily_report.total = F('total') - num
                        daily_report.end_balance = F('end_balance') - num
                    daily_report.save()
            elif original_status != self.status and self.status in [1,2,3] and original_status in [1,2,3]:
                user_balance = UserBalance.objects.get(user=self.user)
                daily_report = UserAssetDailyReport.objects.get_or_create(user=self.user,target_date=today)[0]
...
class PickupDetail(models.Model):
    ...
    trade_type = models.IntegerField(choices=COMMISSION_BUY_TYPE, default=1, verbose_name=u'出售类型')
    ...
    logistics_company = models.CharField(max_length=255, blank=True, null=True, verbose_name=u'物流公司')
    logistics_no = models.CharField(max_length=255, blank=True, null=True, verbose_name=u'物流单号')
    refuse_desc = models.CharField(blank=True, null=True, default='', max_length=1000, verbose_name=u'驳回原因')
    deal_datetime = models.DateTimeField(blank=True, null=True, verbose_name=u'办理日期')
    logistics_date = models.DateTimeField(blank=True, null=True, verbose_name=u'发货日期')
...
class UserProduct(models.Model):
    ...
    def _get_total_price(self):
        try:
            total_price = self.quantity * self.overage_unit_price
        except:
            total_price = 0
        return total_price
    ...
    def save(self, *args, **kwargs):
        ...
        # current_user_products = UserProduct.objects.filter(product=self.product, user=self.user)
        # current_user_products.update(quote_quantity=self.quote_quantity)
        super(UserProduct, self).save(*args, **kwargs)

    def custom_save(self):
        super(UserProduct, self).save()
...
class ProductOrder(models.Model):
    ...
    pickup_list = models.ForeignKey(PickupList, blank=True, null=True, related_name='order_pickup_list', db_index=True,
                                 verbose_name=u'提货单')
    province = models.ForeignKey(Province, blank=True, null=True, related_name='order_province',
                                 verbose_name=u'省')
    city = models.ForeignKey(City, blank=True, null=True, related_name='order_city',
                                 verbose_name=u'城市')
    addr = models.CharField(blank=True, null=True, default='', max_length=1000, verbose_name=u'订单地址')
    receive_addr = models.ForeignKey(ReceivingAddress, blank=True, null=True, related_name='order_receive_addr',
                                 verbose_name=u'收货地址')
    pickup_addr = models.ForeignKey(PickupAddr, blank=True, null=True, related_name='order_pickup_addr',
                                 verbose_name=u'订单提货点')
...
class OrderInfo(models.Model):
    ...
    order_pickup_detail = models.ForeignKey(PickupDetail, blank=True, null=True, related_name='order_pickup_detail', db_index=True,
                                 verbose_name=u'提货单详细')
    class Meta:
